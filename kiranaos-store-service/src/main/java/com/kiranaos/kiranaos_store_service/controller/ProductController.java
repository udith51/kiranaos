...
public class ProductController {
    ...
    @PostMapping
    public ResponseEntity<ProductResponse> createProduct(@RequestHeader("X-Owner-Id") UUID ownerId, @Valid @RequestBody CreateProductRequest createProductRequest) {
        return ResponseEntity.status(HttpStatus.CREATED).body(productService.createProduct(ownerId, createProductRequest));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ProductResponse> getProduct(@PathVariable UUID id, @RequestHeader("X-Owner-Id") UUID ownerId) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProductResponse> updateProduct(@PathVariable UUID id, @RequestHeader("X-Owner-Id") UUID ownerId, @RequestBody UpdateProductRequest updateProductRequest) {
        return ResponseEntity.status(HttpStatus.OK).body(productService.updateProduct(id, ownerId, updateProductRequest));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<MessageResponse> deleteProduct(@PathVariable UUID id, @RequestHeader("X-Owner-Id") UUID ownerId) {
        return ResponseEntity.status(HttpStatus.OK).body(productService.deleteProduct(id, ownerId));
    }

    @GetMapping("/category/{category}")
    public ResponseEntity<List<ProductResponse>> getAllProductsByCategory(@PathVariable String category, @RequestHeader("X-Owner-Id") UUID ownerId) {
    ...
    public ResponseEntity<ProductImportResponse> bulkImport(@RequestHeader("X-Owner-Id") UUID ownerId, @RequestParam("file") MultipartFile csvFile) {
        return ResponseEntity.status(HttpStatus.CREATED).body(productService.importProducts(ownerId, csvFile));
    }
}
